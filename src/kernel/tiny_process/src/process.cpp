...
Process::Process(const string_type &command, const string_type &path,
        ...
        closed(true), process_finished(process_finished), read_stdout(read_stdout), read_stderr(read_stderr),
...
void Process::kill(bool force) {
    std::lock_guard<std::mutex> lock(close_mutex);
    if (data.id > 0 && !closed) {
        ::kill(-data.id, (force) ? SIGTERM : SIGINT);
    }
}
